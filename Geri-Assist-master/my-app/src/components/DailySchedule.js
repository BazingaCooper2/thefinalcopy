                                ...
                                <React.Fragment key={employee.emp_id}>
                                    ...
                                    <div className="employee-row">
                                        ...
                                        <div className="employee-time-slots">
                                            {dailyShift && dailyShift.shift_start_time && dailyShift.shift_end_time && (() => {
                                                // Parse time strings (format: "HH:MM")
                                                const [startHour, startMin, startSec] = dailyShift.shift_start_time.split('T')[1].split(':').map(Number);
                                                const [endHour, endMin, endSec] = dailyShift.shift_end_time.split('T')[1].split(':').map(Number);
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="shift-block-content">
                                                            <div className="shift-time-range">
                                                                <i className="bi bi-clock"></i>
                                                                {dailyShift.shift_start_time.toString().split('T')[1].slice(0, 5)} - {dailyShift.shift_end_time.toString().split('T')[1].slice(0, 5)}
                                                            </div>
                                                            <div className="shift-location-info">
                                                                <i className="bi bi-geo-alt"></i> {employee.service_type || 'Staff'}
                                                            </div>
                                    ...
                                    {isExpanded && hasClientShifts && (
                                        <div className="client-shifts-row">
                                            ...
                                            <div className="employee-time-slots">
                                                {clientShifts.map((shift, shiftIndex) => {
                                                    const client = getClient(shift.client_id);
                                                    const [startHour, startMin, startSec] = shift.shift_start_time.split('T')[1].split(':').map(Number);
                                                    const [endHour, endMin, endSec] = shift.shift_end_time.split('T')[1].split(':').map(Number);
